...
Milim embeds a Rust backend with a local HTTP API and brings provider-agnostic dev chat, encrypted provider management, Codex, Claude CLI, OpenCode, and Pi account runtimes, opt-in isolated thread worktrees, review comments, an in-app GitHub PR cockpit, agents/tools, memory/RAG, artifact previews, and outbound privacy controls into one app.
...
- Milim uses Claude's documented `claude -p` stream-JSON interface and matches structured tool results to their originating calls.
...
Pi is also installed and authenticated separately. Run `/login` inside Pi; Milim never reads or stores Pi credentials. Embedded Pi turns use the documented JSONL RPC protocol with exact provider/model selection and resumable Milim-owned session ids. They disable discovered extensions so arbitrary startup code cannot bypass Milim safety: Plan/Guarded are read-only, Review uses a temporary exact-arguments approval extension, and Open exposes Pi's built-in tools without prompts. Pi's normal context files, prompt templates, and skills remain available for folder-backed chats; no-folder chats run without tools or project context.

OpenCode no-folder chats use a private managed ACP directory for protocol compatibility with native filesystem tools disabled; Milim-owned tools remain available.
...
| Account runtimes | Codex, Claude, OpenCode, and Pi status/run endpoints; `GET /account-runtimes/updates`, `POST /account-runtimes/{runtime}/update` |
...
| Memory / RAG | `POST /memory/ingest`, `/memory/search`, `/memory/register`, scoped hybrid `/memory/graph/search`, plus scope/node routes |
...
| Google Workspace (desktop-internal) | Authenticated connection status, system-browser Picker, selected-file registry, honest confirmed/unconfirmed disconnect results, MIME-aware preview, and bounded media proxy routes under `/google-workspace/*` |
...
| Codex account runtime | `GET /codex/account`, `POST /codex/login/device`, `/codex/login/chatgpt-device`, `/codex/login/api-key`, `/codex/logout`, `GET /codex/rate-limits`, `GET /codex/models`, read-only `GET /codex/threads` and `/codex/threads/{id}`, `POST /codex/run` |
| Installed Claude CLI | `GET /claude/status`, `POST /claude/run` |
| Health | `GET /health` |

Built-in tool safety is enforced server-side. Each run captures its workspace and preview target. Plan is read-only, Guarded exposes only tools declared read-only, Review pauses each mutating, command, or unknown invocation on an inline exact-arguments Approve/Deny card, and Open executes eligible calls automatically. Review approvals are unguessable, one-shot, and canceled when the stream stops; the legacy whole-run grant remains available only for explicit non-interactive API callers. Direct `POST /mcp/call` is read-only even though `GET /mcp/tools` returns the full catalog for agent configuration. New chats default to Review, and changing a chat's workspace folder resets approval to Review so consequential tools remain available behind exact-call approval without carrying Open permission across project boundaries. Filesystem tools reject traversal through symlinks/junctions, use atomic replacement for writes, and support bounded ranged reads. `http_fetch` permits public HTTP(S) destinations only. Host and Docker commands have time/output limits; Docker runs also use no-network, memory, CPU, PID, capability, and read-only-root restrictions by default.

Desktop release builds can enable selected-file Google Workspace access with `MILIM_GOOGLE_OAUTH_CLIENT_ID` and `MILIM_GOOGLE_OAUTH_CLIENT_SECRET`. The desktop OAuth client uses PKCE and Google's system-browser desktop Picker; its installed-app client secret is build configuration and is never a user credential. The connector requests only `drive.file` and stores one refresh token plus the selected/Milim-created file registry in AES-GCM encrypted local state. Picker supports selecting many files in one confirmation; selecting a folder itself is intentionally disabled because Google does not extend `drive.file` authorization to its children. Folder URLs open Picker filtered to that folder so its files can be selected together; selected folder entries open in the active Milim tab, middle-click or Ctrl/Cmd-click opens a new Milim tab, and the context menu exposes open/new-tab/copy/external actions. The trailing external-link action alone opens Google directly. Native Sheets, Docs, and Slides previews apply manual changes immediately, then autosave them through a serialized background queue after a short idle period or on blur/navigation; failed saves keep the local change visible and offer Retry. Docs edit contiguous rendered paragraphs in one native surface on click, allowing selections and edits across paragraph boundaries while tables and embedded objects remain view-only. One contextual command bar combines search, paragraph/list actions, selected-text formatting, save status, outline, and zoom. Slides expose native in-place text boxes without a separate edit mode, preserve mixed character formatting in the active editor, apply selected-text formatting locally before save, and keep the slide unobstructed by floating panels. Editable slide elements can be selected, moved, corner-resized with edge/center snapping guides, duplicated, or deleted; slides can be created, duplicated, deleted, and reordered from the same command bar. Slides keep optimistic text and transformed visuals mounted while preview metadata and thumbnails reconcile in the background, then swap the refreshed thumbnail without blanking the canvas. Thumbnail loading and failures use a neutral surface rather than presenting flattened text as the slide. Sheets support pasted ranges and row/column insertion or confirmed deletion, and Slides add a collapsible thumbnail rail and clean fullscreen presentation mode with keyboard navigation. The first parentless create or upload lazily creates a Milim Drive folder; later parentless Milim-created files go there and are automatically added to the registry. Provider models and Codex, Claude, OpenCode, and Pi account runtimes receive the same bounded Drive, Sheets, Docs, and Slides tools—including slide and element structure/transform operations—through Milim's per-turn gateway. Reads follow Guarded, edits and Drive management are mutating, and workspace transfers are commands. Files removed from Milim remain in Drive, deletion is trash/restore only, sharing is limited to named users/groups, and Google credentials plus the file registry are excluded from backups.

Slides saves no longer trigger an immediate preview reload: optimistic text and transformed element layers remain mounted across thumbnail navigation while writes finish in the background. Selected elements drag directly from their body or selection border and use four quiet corner resize handles; dragging across empty slide space selects every intersecting text box or element for grouped moving or resizing.

The Sheets inspector uses one compact controls row, an editable autosaving formula bar, contextual row/column actions, and bottom worksheet tabs so the grid keeps most of the inspector height.
...
Desktop provider keys, Google authorization, MCP secrets, and mobile-companion device keys are encrypted with one per-data-root master key held in Windows Credential Manager or macOS Keychain. If the OS vault is unavailable, Milim reports and uses a permission-restricted local key. The first launch after this storage upgrade migrates and removes the older adjacent key files and plaintext mobile-device state only after verified re-encryption; that credential migration is intentionally one-way.
...
crates/
  ...
  milim-memory/     embedding vector store, scoped hybrid memory, RAG
...
Desktop Tauri commands automatically load an ignored repo-root `.env.local` when it exists. Local Google Workspace builds use the same two `MILIM_GOOGLE_*` variables documented above; GitHub release builds continue to receive them from Actions secrets.

Shipped UI:

- Unified first-run setup follows Model, Defaults, optional Context, and Ready. The expanded sidebar and unified inspector use inset, theme-rounded surfaces. The collapsed sidebar defaults to one full-height rail, with optional split top/bottom or compact centered layouts in Appearance settings. Advanced features are always available; the collapsed sidebar Tools launcher opens MCP servers, Skills, Schedules, Pull requests, and the media manager.
- Skills use progressive disclosure. Automatic and Custom modes rank at most three enabled candidates and send compact name/description metadata; the model loads a complete body only when needed through read-only `milim_skill_search` and `milim_skill_read` tools. Custom is an allowlist, explicit `@Skill Name` or `/Skill Name` references load that skill immediately, and prompt limits omit later skills whole instead of slicing instructions. Account runtimes keep their native skill catalogs and receive Milim-managed skills through the per-turn gateway without duplicate eager injection.
- The first product loop is dev chat: connect a provider or account runtime, pick a workspace folder, and start from the configured workspace preference or explicitly choose the current checkout or an isolated worktree from the New chat split button. Ask for an edit or test run, switch models inline, and continue the same thread without resetting workspace context, memory, previews, artifacts, approvals, or queued messages.
- Desktop motion is frequency-aware: shortcut-driven search, model, context-menu, and thread changes stay instant; truncated model and thread labels reveal themselves after a one-second hover; occasional panels use short interruptible transitions; reordered rows track the pointer directly; and reduced-motion preferences remove spatial movement and spinning while retaining perceivable status.
- After onboarding, the first startup of a release can show a short, keyboard-accessible stack of update cards sourced from the bundled `apps/desktop/src/update/releases.json`. Dismissing the stack records that exact installed version locally; Developer settings can replay the current cards without changing that state.
...
- Provider/local chats and account runtimes receive the visible Milim browser tab's untrusted URL/title metadata and can use Milim-owned tools through the same approval policy. Account runtimes retain their native filesystem and shell instead of receiving duplicate host tools. They can also list, test, save, enable/disable, and remove Milim-managed MCP servers: Guarded permits read-only calls, Review pauses consequential calls on the existing exact-arguments approval card, and Open executes them directly. Credential-looking environment variables are accepted only as value-free placeholders; enter their values afterward in the encrypted MCP Manager. Newly connected tools become callable on the next chat turn.
- Review approval cards attach to the composer while awaiting a choice; the transcript keeps one compact request-and-decision event for history. A completed agent round that successfully used a Sheets, Docs, or Slides edit tool refreshes the open Google Workspace preview across provider and account-runtime lanes.
- PNG, JPEG, WebP, and GIF attachments up to 2 MB each are forwarded as real image content to provider chat, server-side agent runs, provider-backed schedules, Codex app-server turns, Claude CLI turns, and Pi RPC turns; no OCR or text fallback substitutes for the pixels. Outbound turns keep the latest images plus complete recent image-bearing turns within a 20 MiB encoded-image budget; older pixels remain visible in the transcript but become an omission note in model context. Requests above 30 MiB fail locally with removal/compaction guidance instead of reaching the server's 32 MiB cap. Codex uses temporary per-turn `localImage` files that are deleted when the turn ends, while Claude and Pi receive native base64 image blocks. Oversized, unsupported, unreadable, or legacy scheduled images without stored data fail visibly. Desktop attachment reads remain limited to files picked in the native dialog or workspace-relative files under the selected folder.
- Provider onboarding for OpenAI, OpenRouter, Groq, Ollama, LM Studio, custom OpenAI-compatible remotes, Anthropic, Gemini, and Replicate/fal media with encrypted key storage and model capability metadata, including native local vision/tool-use labels where available; account-runtime onboarding for Codex, the installed Claude CLI, OpenCode, and Pi, with per-chat native session persistence, consent-gated Claude session recovery, Codex transcript recovery, structured approvals, Milim approval modes, and visible tool events. The model chip is the sole visible model selector; branded provider/runtime icons identify its route while every picker row names the provider or nested runtime route, such as `Pi · GitHub Copilot`, and omits those redundant prefixes from the primary model label. The picker also shows setup status, capabilities, Favorites, per-model reasoning effort, and whether the next turn is plain chat, Milim tools, an account runtime, or media. Provider and runtime groups can be collapsed, with one shared layout persisted across every picker surface.
...
- Memory/RAG with Personal and Git-stable Project libraries, exact-term plus embedding retrieval fused with reciprocal rank fusion, and at most five provenance-labeled entries inside a hard 1,024-token context budget per enabled turn. Retrieval keeps stable and legacy exact-folder scope reads, archive/recovery controls, legacy thread-memory recovery, explicit tool-capable writes, and provider-routed embeddings behind the same remote privacy gate; lexical recall remains available when embeddings do not.
- Workspace and Git panels with guarded fetch/commit/sync actions, a review-first local diff workspace for all, unstaged, staged, last-turn, commit, and branch comparisons, and a hideable, resizable changed-file tree on the left that navigates the unified diff. Git also supports local branch switching and creation, model-generated commit subjects when the message is left blank, turn-level Git worktree checkpoints with restore actions, a unified Preview / Code / Git / Workers inspector, and a persisted thread-local Context card with a compact Worker summary plus independently collapsible environment, task, live activity, context, and sources sections. Context groups the current prompt estimate, free tokens, fixed-context categories, repository-rule files, and warnings under a collapsed Prompt context disclosure; provider-reported usage remains separately labeled as cumulative usage. Sources show five filename-only, single-line entries at a time and open their attachment, artifact, or memory target. Context's reserved 300px slot pushes the transcript while its scrollbar stays at the thread view's right edge, can coexist with the inspector on wide layouts, and becomes mutually exclusive with it on narrow layouts. A workspace launcher appears only for folder-backed threads and opens the active folder in installed local tools; project context menus can also open the project folder directly in the platform file manager and clear unread state across that project. Filesystem/shell tools remain rooted to the selected folder, generated-file diff review/apply/save-to-workspace flows use selected artifact revisions, Docker sandbox runs remain isolated, preview DOM tools require an active ready surface, optional computer-use controls OS input, and Review/Guarded/Open gate tool execution.
- The latest code-changing turn ends with a compact changed-files card showing aggregate and per-file line counts. **Review changes** opens that exact checkpoint diff in the Git inspector; **Undo** restores the pre-turn checkpoint, removes the assistant response, retains the original request, and clears native runtime state that no longer matches the workspace.
- The Git inspector switches between local Changes and the current branch's GitHub PR. The PR cockpit renders Markdown plus sanitized GitHub HTML in descriptions, reviews, and comments, with reviewers, normalized checks, merge readiness, and viewer-aware merge permissions; linked badges and bounded attachment images remain visible without allowing active HTML. It can draft an agent readiness pass, comment, review, mark drafts ready, or merge with an explicit method and head-SHA guard. The resizable, persisted Pull requests tool adds a searchable account-wide authored/review-requested list with GitHub user and App avatars, a persisted draggable list/detail divider, and Summary, Timeline, and Code views. Saved list and detail data render immediately while refreshes upsert changes in the background, and the same guarded comment, review, ready, and merge actions remain available. Visible isolated-worktree threads and the active shared checkout expose live accessible PR-state badges in the sidebar. GitHub actions require the separately installed and authenticated `gh` CLI.
...
- Image, video, and prompt-to-music generation through configured OpenRouter, fal, and Replicate providers, with kind-aware model discovery, schema controls, status polling, native music playback, and window-filling in-app previews for generated images and videos. Chat remains the iterative generation surface, while the standalone Media Studio pairs the shared composer surface with a selected-output stage and collapsible searchable library sidebar. Successful and failed runs from both surfaces are indexed in `~/.milim/media/index.json`; downloaded files live under `~/.milim/media/files/<library-id>/` until explicitly deleted. Failed local saves keep their remote preview and can be retried from the studio. OpenRouter image generation is live-verified; video/music and fal/Replicate music have mocked adapter coverage but still require separate credentialed smoke verification.
...
- Searchable settings organized as Preferences, Workflows, Data & devices, and Application, with a dedicated Google Workspace section, direct control results, and actionable-only badges. Workflow preferences cover startup/panel restore, opt-in native notifications, manual-install update policy, inherited or configured new-chat defaults, Smart/Pinned/Hidden quick actions with project overrides, Automatic/Manual/Off composer suggestions, local-only bounded ranking and cross-chat prompt history, live model/agent defaults and safe unavailable-model policies, workspace opener/worktree behavior, JSON/Markdown export defaults, and transactional `.milim-backup.json` backup/restore with a pre-restore recovery snapshot. Opt-in composer ghost completion is local-first, sends only current composer text, excludes account runtimes, and requires explicit privacy/cost confirmation for a remote current-provider model. Appearance can hide the default-on local activity ridgeline above an empty chat's composer; themes retain contrast validation, animated backgrounds, and default-off local sounds. Configurable shortcuts include composer suggestions, `Ctrl/Cmd+K` for the command-and-chat palette, and `Ctrl+Tab` for recent threads. On macOS, `Cmd+W` closes an active URL-browser tab or hides the app window to its menu-bar icon when the URL browser is not handling the shortcut. The app also includes resizable panels, frameless controls, close-to-tray background mode, bounded local logs, and checksum-verified Windows/macOS updates that download and stage in the background by default with an opt-out, but never install or restart until the user clicks the ready update action.
