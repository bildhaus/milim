...
- The Git inspector switches between local Changes and the current branch's GitHub PR. The PR cockpit renders Markdown description/comments, reviewers, normalized checks, and merge readiness; it can draft an agent readiness pass, comment, review, mark drafts ready, or merge with an explicit method and head-SHA guard. The resizable, persisted Pull requests tool adds a searchable account-wide authored/review-requested list with GitHub avatars, a persisted draggable list/detail divider, and Summary, Timeline, and Code views. Saved list and detail data render immediately while refreshes upsert changes in the background, and the same guarded comment, review, ready, and merge actions remain available. Visible isolated-worktree threads and the active shared checkout expose live accessible PR-state badges in the sidebar. GitHub actions require the separately installed and authenticated `gh` CLI.
