...
Slides saves no longer trigger an immediate preview reload: the optimistic canvas stays mounted while writes finish in the background. Selected elements drag directly from their body or selection border and use four quiet corner resize handles; dragging across empty slide space selects every intersecting text box or element for grouped moving or resizing.
...
- Unified first-run setup follows Model, Defaults, optional Context, and Ready. The expanded sidebar and unified inspector use inset, theme-rounded surfaces, while the collapsed rail splits its top actions and bottom Tools/Settings into two matching floating surfaces. Advanced features are always available; the collapsed sidebar Tools launcher opens MCP servers, Skills, Schedules, Pull requests, and the media manager.
