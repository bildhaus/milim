...
| Codex, Claude, OpenCode, or Pi CLI not found | Launches from the Dock, Finder, or a desktop entry now also read your login shell's `PATH` (so nvm and fnm installs are found). Otherwise install with the command shown on that runtime's Providers page, or use **Locate binary...** to choose the executable. |
