...
Milim-native uses the registry's effect metadata. Codex keeps `on-request` approval with a workspace-write sandbox and relays app-server command, file, and permission requests. Review presents those requests inline, while Open auto-approves them so protected Git metadata operations such as creating branches and commits can proceed. Claude uses a temporary per-run Streamable HTTP MCP permission tool and deletes its run token/configuration on completion. A runtime that cannot support its approval protocol fails Review instead of silently switching modes. API callers may still set `tool_approval_grant: true` as an explicit whole-run compatibility grant; streamed desktop runs do not.
