...
Every listener validates the request `Host` header and answers `403` for names it does not serve, so a web page cannot reach milim by pointing its own DNS name at your machine. A loopback listener accepts `localhost`, `127.0.0.1`, and `[::1]` on any port. `milim serve --expose` and the mobile listener also accept IP literals, `*.local`, `*.ts.net`, and this machine's host name. Add other names, such as a reverse proxy's, with the comma-separated `MILIM_ALLOWED_HOSTS` environment variable.
...
| Memory | `POST /memory/ingest`, `POST /memory/search`, `POST /memory/register`, `POST /memory/graph/search`, `POST /memory/benchmark`, `GET /memory/scopes`, `GET /memory/nodes`, node update/delete/archive/review/restore routes |
...
`POST /memory/benchmark` accepts a model, bounded `top_k`, optional archive inclusion, and labeled cases shaped as `{ name?, query, relevant_node_ids, scopes? }`. It executes the production hybrid search and returns per-case retrieved IDs, first relevant rank, recall@k, reciprocal rank, macro recall@k, and mean reciprocal rank. It is read-only. `POST /memory/nodes/{id}/review` timestamps an active memory as reviewed; archive revokes it from ordinary retrieval, and `POST /memory/nodes/{id}/restore` clears the archive and marks it reviewed.
