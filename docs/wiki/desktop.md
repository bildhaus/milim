...
| Settings | Settings search returns individual controls, jumps to the matched row, and section navigation shows warning status for incomplete setup. Appearance includes default-off, locally synthesized sounds with separate attention, active-chat completion, sound-choice, preview, and optional interaction-feedback controls. Attention covers pending tool approvals, proposed worker plans, and terminal errors; hidden/background work stays silent. |
