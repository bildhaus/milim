...
Each runtime can also use an explicit executable. **Locate binary...** on its Providers page (or in onboarding) saves an absolute path in the canonical store (`PUT /account-runtimes/{runtime}/binary` with `{ "path": "..." }`, or `null` to reset; `GET /account-runtimes/binaries` lists them). The override wins for turns, status checks, model discovery, and updates. Its folder is placed first on the child's `PATH` so a Node launcher still finds the `node` beside it. An override that no longer points at an executable is ignored until it is fixed or reset.
...
Each runtime's Providers page shows the detected CLI version and compares it with the latest stable version published for that CLI. An available update becomes the page's one emphasized action (**Update to vX**) and a rail hint, and Overview offers **Update all**; a current runtime shows a quiet **Up to date** beside its version. If the release check is unavailable, the updater stays available in the page's actions menu. Updating requires a second confirmation, then invokes that runtime's own updater (`codex update`, `claude update`, `opencode upgrade --pure`, or `pi update self --no-approve`) and rechecks the installed version. Finish active turns first. milim does not replace these tools' installers, credentials, or standalone configuration.
...
`approval.resolve` accepts an optional `payload.scope` of `once` (the default when omitted) or `thread` ("Allow for this chat"); the desktop's `POST /tool-approvals/{id}` forwards the same optional `scope`. A thread-scoped approval of a command or file-change request records a per-thread rule in canonical Rust state and approves later matching requests in that thread automatically, from any runtime. Command-bearing requests are keyed by the exact command string, and shell-style tools without one are rejected rather than allowed by name; other tools are keyed by tool name. Codex additionally receives `acceptForSession` and OpenCode `allow_always` when the request advertises them; Claude, Pi, and milim's provider tool loop rely on the milim rule. Permission elevations and MCP input or authorization stay one-shot. An auto-approved request's `approval_requested` timeline item carries `auto_approved: { "scope": "thread", "allowance": "<key>" }`. `GET /control/v1/threads/{id}/approval-allowances` lists the rules, `DELETE` on the same path revokes all of them or `{ "keys": [...] }`, and changes emit an `approval_allowances.updated` control event. Changing the thread's workspace folder or deleting the thread discards its rules.
...
When a canonical chat has reciprocal links, the frozen milim context exposes the same `linked_thread_list`, `linked_thread_read`, and eligible `linked_thread_send` / `linked_thread_wait` MCP tools to Codex, Claude, OpenCode, and Pi. List/read use the accepted run's immutable target grants and bounded visible transcript projection. Send and wait are absent in Plan and Guarded, send is approval-gated in Review and immediate in Open, and both remain subject to the selected Agent's tool allowlist. The destination always runs with its own frozen account/provider runtime, workspace, privacy, Agent, and approval configuration. A run may wait only on an exchange it created, for at most 90 seconds per call; a successful wait consumes the reply into that run, while a timeout leaves the late reply durable. Unwaited replies are claimed when the origin's next turn actually starts.
...
Managed Workers inherit an account-runtime parent's adapter and model, but not its native session. Each Worker starts a fresh native session that milim does not bind or retain for reuse, runs under Guarded read-only access, disables further milim Worker delegation, and normalizes the harness stream into the existing Worker Run ledger. This keeps parallel Worker transcripts and native session locks isolated from the parent.
