...
When a canonical chat has directional links, the frozen Milim context exposes the same `linked_thread_list`, `linked_thread_read`, and eligible `linked_thread_send` MCP tools to Codex, Claude, OpenCode, and Pi. List/read use the accepted run's immutable target grants and bounded visible transcript projection. Send is absent in Plan and Guarded, approval-gated in Review, immediate in Open, and still subject to the selected Agent's tool allowlist. The destination always runs with its own frozen account/provider runtime, workspace, privacy, Agent, and approval configuration; a native origin session receives claimed mailbox replies only when its next turn actually starts.
