...
Milim's in-app Preview inspector is separate from the OpenAI Browser plugin. Codex turns use the injected `preview_open_url` Milim MCP tool to open public HTTPS or loopback HTTP pages in the visible task; local previews start their development server before opening its loopback URL. Once the page is ready, the existing scoped preview tools provide DOM inspection and interaction.
