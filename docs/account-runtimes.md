...
The response is SSE. Every JSON event has `schema_version: 1`, a per-run `run_id`, increasing `seq`, `at_ms`, `harness_id`, and one canonical application event. The event vocabulary covers native session and turn identity, text and reasoning deltas, tool start/update/finish, approval lifecycle, usage and limits, generated images, native workers, runtime and recovery notices, and exactly one completed, failed, or cancelled terminal event. That terminal event closes the desktop response immediately while the native adapter gets a bounded background drain for cleanup. Bridge protocol names, ACP update tags, and JSON-RPC correlation ids do not cross this desktop boundary.
